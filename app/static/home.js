...
confirm_delete_result_modal.addEventListener("close", async () => {
    if (confirm_delete_result_modal.returnValue != "cancel") {
        try {
            await fetch(`api/v1/cbmc/tasks/${confirm_delete_result_modal.returnValue}`, {
                method: "DELETE",
            });
        } catch (err) {
            console.error(err);
...
async function run_all_proofs() {
    ...
    try {
        response = await fetch(`api/v1/cbmc/tasks`, {
            method: "POST",
        }).then((res) => res.json());
    } catch (err) {
        console.error(err);
...
async function cancel_proof_run() {
    let response;
    try {
        response = await fetch(`api/v1/cbmc/tasks/current`, {
            method: "DELETE",
        });
    } catch (err) {
        console.error(err);
